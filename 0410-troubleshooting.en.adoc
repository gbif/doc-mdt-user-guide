...
Users with prepared datasets ready to process can skip this section and go to: <<detailed_select_mdt>>.

Users without prepared datasets can use ../example_data/example_data2.current.en.xlsx[example_dataset_2] – a real metabarcoding dataset of surface seawater metazoan communities, that has been modified to better illustrate some features of the MDT. 

NOTE: This example dataset was formatted for publishing to OBIS. Many OTUs have not been possible to identify (_or_ were identified to a species not on the WoRMS Checklist) and are simply named "Incertae sedis" (uncertain placement). Read a more detailed description of the dataset <<example2, here>>. You can also explore the dataset in the MDT https://mdt.gbif-uat.org/dataset/3d4e6748-df68-4047-ab04-ac3c3baa7840/review[here].
...
** The *Taxonomy* table contains IDs correspoding to those (rownames) in the *OTU_table*, the sequence, and taxonomic information.
...
Select a suitable <<installations,MDT installation>> for the processing:
