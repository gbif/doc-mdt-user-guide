...
At this step, a <<dwc-a>> file is produced, which can be published to GBIF. In the MDT demo, the archive can (only) be published to the GBIF *test* environment (UAT) for users to preview a real GBIF.org publication.
...
If using this quick start guide as suggested, you will be using the GBIF Demo MDT. The publishing step (step 7) is *not enabled* for the GBIF Demo MDT, and step 7 will appear as in the figure below. Read about the publishing step in the <<detailed_guidance>>.

.The *Publish* step is not enabled for the GBIF Demo MDT. If you by chance prepared a real dataset using the GBIF Demo MDT, it is recommended to redo the processing in a proper MDT installation, unless this would for some reason be too problematic. In that case you can use the link on the page: "Ready to publish a dataset? Reach out to the administrator for assistance".
