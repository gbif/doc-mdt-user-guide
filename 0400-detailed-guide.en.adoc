...
Select a suitable <<installations,MDT installation>> for the dataset to be processed and possibly published.

* Still learning testing? Use the https://sandbox-mdt.gbif-uat.org/[MDT Sandbox (Demo Installation)].
* Got a real dataset? Identify and use the most relevant MDT installation for you #how can we guide here??#
...
Some repetition from simple quick start guide.
...
* _Sample section_: the MDT automatically identified and mapped 4 fields from the uploaded Samples table (second column) and 5 global term/values from the Study table (third column) to identically named Darwin Core terms (first colum). +
* _Taxon section_: the MDT automatically identified and mapped 4 fields from the uploaded Taxonomy table. 
****
...
IMPORTANT: For real datasets it is higly recommended to provide rich and detailed information to improve and extend discoverability and reuse, and facilitate the users can better assess fitness for purpose when using the data.

TIP: Toggle "Show help" to get guidance text for the fields.

.*Edit metadata*. In this section you provide dataset information/metadata in defined sections (left panel): _Basic Metadata, Geographic Coverage, Taxonomic Coverage, etc._ For real datasets it is important to provide rich and detailed information to improve discoverability and reuse. 
...
****
*Example dataset* +
Explore the metadata sections to get familiar with options. When working with a real dataset, please refer to the full guidance in this section and to the section on <<preparation_structure>>.
...
image::img/advanced_uat.png[][pdfwidth=20%,scaledwidth=20%]
...
.To publish their first dataset on GBIF, the user's institution must be registered as a data publisher, and the user must be associated with the institution in the MDT.
...
When a user has been associated with a publishing organization in the MDT, the user can publish datasets to GBIF.org.
...
. If relevant (users will usually know), select a Network that  the dataset should be associated with. NB: selecting "OBIS" will notify OBIS that the dataset is available so they can index it. (#is that how this works?#)
. Press *Publish to gbif.org*

image::img/dg_publish_publish.png[]
.To publish a dataset to GBIF, the user's institution must be registered as a data publisher, and the user must be associated with the institution in the MDT.
...
NOTE: User that have prepared datasets using the GBIF Conversion MDT should try to make sure that there is not a more relevant MDT installation hosted by a GBIF node. #how can we guide here?#
...
By downloading dataset from the MDT and publishing elsewhere, the possibility for easy updating, re-processing and visualization of the dataset in the MDT is lost. Also, archives dowloaded and modified (e.g. in the IPT), will not subsequently be possible to easily modify or update using the MDT.

The MDT produces fully publishable Darwin Core Archives, with no need for further changes or additions in the IPT. You can validate the produced <<dwc-a>> in the https://www.gbif.org/tool/81281/gbif-data-validator[GBIF data validator].
